...
use crate::states::worker::Worker;
use anchor_lang::prelude::*;

pub fn process_payroll<'info>(
    ctx: Context<'info, ProcessPayroll<'info>>,
    cycle_timestamp: u64,
) -> Result<()> {
    let num_workers = ctx.accounts.org.worker_count as usize;
    ...
    for i in 0..num_workers {
        ...
        if worker.last_paid_at < cycle_timestamp {
    ...
    for i in 0..num_workers {
        ...
        if worker.last_paid_at < cycle_timestamp {
            ...
            // Update worker data
            worker.last_paid_at = cycle_timestamp;
